...
import 'package:fwo/src/presentation/logic/controllers/send_notification.dart';
import 'package:fwo/src/presentation/logic/models/user_model.dart';
import 'package:get/get.dart';

import '../../../core/config/Translation/local.dart';
...
class OrderController extends GetxController {
  ...
  sabmitOrder(
    ...
    var errorCode = await handleFirebaseError(() async {
      ...
      await _api.sabmitOrder(data).then((value) async {
        String t = LocalStrings().keys['ar_AE']?[user?.type ?? '']??'';
        await SendNotification().sendNotification(
            title: 'طلب جديد',
            body:
                "طلب $t ${user?.name??''} كمية $quantity لتر"
);
  ...
  addSuggestionsAndComplaint(String text) async {
    var errorCode = await handleFirebaseError(() async {
      ...
      await _api.addSuggestionsAndComplaint(data).then((value) async {
        await SendNotification()
            .sendNotification(title: 'اقتراحات وشكاوى جديدة', body: '$text');
        ...
        Get.dialog(AlertDialog(
          ...
          content: Row(
            ...
            children: [
              Text(
                'Suggestion and Complaint submitted successfully'.tr,
  ...
  renewContract() async {
    var errorCode = await handleFirebaseError(() async {
      ...
      await _api.renewContract(data).then((value) async {
        var e = (userBox.get('user') as UserModel);

        await SendNotification()
            .sendNotification(title: 'تجديد عقد الشركة', body: '${e.name}');
